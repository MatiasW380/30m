...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (body.trade) {
      ...
      metrics.best_trade =
        metrics.best_trade === null
          ? trade.roi
          : Math.max(metrics.best_trade, trade.roi);
      metrics.worst_trade =
        metrics.worst_trade === null
          ? trade.roi
          : Math.min(metrics.worst_trade, trade.roi);
      metrics.win_rate =
        metrics.total_trades > 0
          ? (metrics.wins / metrics.total_trades) * 100
          : 0;
    ...
    // Señales del día
    if (body.signals) {
      await redis.set("snip:signals", JSON.stringify(body.signals));
      saved.push("signals");
    }

    // Datos de calibración
    if (body.calib) {
      await redis.set("snip:calib", JSON.stringify(body.calib), { ex: 86400 * 4 });
      saved.push("calib");
    }
